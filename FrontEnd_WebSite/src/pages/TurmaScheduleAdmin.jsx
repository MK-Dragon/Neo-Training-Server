...
import { FaDesktop, FaTools, FaCheckCircle, FaCalendarPlus } from 'react-icons/fa';
...
const TurmaScheduleAdmin = () => {
  ...
  // 3. Fetch Available Rooms for selection
  useEffect(() => {
    const fetchRooms = async () => {
      ...
      const end = `${dayStr}T${String(sorted[sorted.length - 1].hour + 1).padStart(2, '0')}:00:00`;
  ...
  const handleOpenBooking = async () => {
    ...
    } catch (err) { alert("Error loading suggestions"); }
  ...
  const handleConfirmBooking = async () => {
    ...
    setBookingLoading(true);
    try {
      // Loop through each selected slot and add to schedule
      const promises = selectedSlots.map(slot => {
        const body = {
          ...
          DateTime: `${format(slot.day, 'yyyy-MM-dd')}T${String(slot.hour).padStart(2, '0')}:00:00`
        };
        return fetch(`${ServerIP}/api/Shcedule/add-schedule`, {
          method: 'POST',
      ...
      if (allOk) {
        ...
        alert("Some slots could not be booked. Check for overlaps.");
      }
    } catch (err) {
        ...
        <Card.Body>
          <Row className="align-items-center">
            ...
            <Col md={4} className="text-end">
              <Button variant="success" size="lg" disabled={selectedSlots.length === 0} onClick={handleOpenBooking}>
                {bookingLoading ? <Spinner size="sm" /> : <><FaCalendarPlus className="me-2"/> Book Selected</>}
              </Button>
            </Col>
          </Row>
        </Card.Body>
              ...
              <Table bordered className="mb-0 text-center align-middle">
                <thead className="table-dark sticky-top">
                  <tr>
                    ...
                    {days.map(day => (
                      <th key={day.toString()} className="py-2 small">{format(day, 'EEEE')}<br/>{format(day, 'dd/MM')}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                          ...
                          >
                            {session ? (
                              ...
                              <div className="small text-primary fw-bold">
                                {availableRooms.length} Salas<br/>
            ...
            {suggestions.map((s, idx) => (
              ...
              >
                ...
                <div className="text-end">
                   <Badge bg="success" className="me-1">{s.hoursCompleted}h</Badge>
                   <Badge bg="warning" text="dark" className="me-1">{(s.totalDuration - s.hoursCompleted)}h Left</Badge>
                   <Badge bg="secondary">Total {s.totalDuration}h</Badge>
                </div>
              </ListGroup.Item>
            ))}
          </ListGroup>
          ...
          {selectedSuggestion && (
            <div className="bg-white p-3 rounded border shadow-sm animate__animated animate__fadeIn">
              ...
              <Row className="align-items-end">
                <Col md={8}>
                  <Form.Group>
                    <Form.Label className="small fw-bold">Available Rooms for this Time Frame</Form.Label>
                ...
                <Col md={4}>
                  <Button 
                    ...
                    disabled={!selectedRoomId || bookingLoading}
                    onClick={handleConfirmBooking}
                  >
                    {bookingLoading ? <Spinner size="sm" /> : <><FaCheckCircle className="me-2"/> Confirm Booking</>}
                  </Button>
                </Col>
              </Row>
            </div>
